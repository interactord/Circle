...
// MARK: - MarkdownAttributedStringParser
...
public struct MarkdownAttributedStringParser: MarkupVisitor {
  ...
  public func visitCodeBlock(_ codeBlock: CodeBlock) -> NSAttributedString {
    let result = NSMutableAttributedString(
      attributedString: highlighter
        .highlight(codeBlock.code, as: codeBlock.language) ?? NSAttributedString(string: codeBlock.code))
  ...
  let highlighter: Highlightr = {
    let highlighter = Highlightr()!
    highlighter.setTheme(to: "stackoverflow-dark")
    return highlighter
  }()
